...
func (uc *UserCreate) AddBlurtIDs(ids ...uuid.UUID) *UserCreate {
	uc.mutation.AddBlurtIDs(ids...)
	return uc
}

// AddBlurts adds the "blurts" edges to the Blurt entity.
func (uc *UserCreate) AddBlurts(b ...*Blurt) *UserCreate {
	ids := make([]uuid.UUID, len(b))
	for i := range b {
		ids[i] = b[i].ID
	}
	return uc.AddBlurtIDs(ids...)
}

// AddLikdBlurtIDs adds the "likd_blurts" edge to the Blurt entity by IDs.
func (uc *UserCreate) AddLikdBlurtIDs(ids ...uuid.UUID) *UserCreate {
	uc.mutation.AddLikdBlurtIDs(ids...)
	return uc
}

// AddLikdBlurts adds the "likd_blurts" edges to the Blurt entity.
func (uc *UserCreate) AddLikdBlurts(b ...*Blurt) *UserCreate {
	ids := make([]uuid.UUID, len(b))
	for i := range b {
		ids[i] = b[i].ID
	}
	return uc.AddLikdBlurtIDs(ids...)
}
...
func (uc *UserCreate) createSpec() (*User, *sqlgraph.CreateSpec) {
	...
	if nodes := uc.mutation.BlurtsIDs(); len(nodes) > 0 {
	...
	if nodes := uc.mutation.LikdBlurtsIDs(); len(nodes) > 0 {
		edge := &sqlgraph.EdgeSpec{
			Rel:     sqlgraph.M2M,
			Inverse: false,
